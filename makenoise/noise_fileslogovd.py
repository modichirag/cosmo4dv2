...
for M0 in mms:
    ...
    for i in range(16):
        ...
        axis.plot(xx, dg.normal(xx, *res.x), 'k--', label='%0.4f, %0.4f'%(res.x[1], res.x[2]))
        tosave.append([msave[i], msave[i+1], res.x[1], res.x[2]])
        axis.legend()

    ff = mtpath + 'noisehist_logovd_M%02d_na.png'%(10*np.log10(M0))
    if Rsm !=3 : ff = ff[:-4] + '_R%d.png'%Rsm
    fig.savefig(ff)

    fpath = mtpath + 'hist_logovd_M%02d_na.txt'%(10*np.log10(M0))
