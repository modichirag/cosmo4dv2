...
def rsdfacf(zz):
    ...
    print('rsd factor for velocity in data is = ', rsdfac)
    return rsdfac
...
suff = None
#suff = '3x'
rsdfactor = rsdfacf(zz)
print('Rsd factor is %0.2f'%rsdfactor)
...
if rsd:
    if not zz:
        ...
        cat = BigFileCatalog(scratch + '/data/z%02d/L%04d_N%04d_S%04d_05step/dynamic/1'%(zz*10, bs, nc, seed), 
                             header='Header')
    ...
    predict = pm.paint(gridpt+vgrid.reshape(-1, 1)*direct.reshape(1, -1), mass = predictgrid)
...
if rsd:
    datap = pm.paint(hdictf['position'][:num] 
                     + rsdfactor*(direct.reshape(-1, 1)*hdictf['velocity'][:num][:, 2]).T, 
...
#
    ...
    for i, sg in enumerate(sgs):
        ...
        if rsd:
            datasg = pm.paint(hpos[:num] + rsdfactor*(direct.reshape(-1, 1)*hvel[:num][:, 2]).T, mass = hmass[:num])
        else:
    ...
    ##Save
    ff = mtpath + 'noisehist_M%02d_narsd2sm%d.png'%(10*np.log10(M0), Rrsd)
    ff = mtpath + 'noisehist_M%02d_narsd2sm%d.png'%(10*np.log10(M0), Rrsd)
    if Rsm !=3 : ff = ff[:-4] + '_R%d.png'%Rsm
    if suff is not None: ff = ff[:-4] + '%s.png'%suff
    fig.savefig(ff)
    ...
    fpath = mtpath + 'hist_M%02d_narsd2sm%d.txt'%(10*np.log10(M0), Rrsd)
    if Rsm !=3 : fpath = fpath[:-4] + 'R%d.txt'%Rsm
    if suff is not None: fpath = fpath[:-4] + '%s.txt'%suff
    ...
    for j, sg in enumerate(sgs):
        ...
        for i, res in enumerate(fits[j]):
            tosave.append([msave[i], msave[i+1], res.x[1], res.x[2]])
        fpath = mtpath + 'hist_M%02d_sg%02d_narsd2sm%d.txt'%(10*np.log10(M0), sg*100, Rrsd)
        if Rsm !=3 : fpath = fpath[:-4] + 'R%d.txt'%Rsm
        if suff is not None: fpath = fpath[:-4] + '%s.txt'%suff
        np.savetxt(fpath, np.array(tosave), header ='Mmax, Mmin, mean(offset: data-model), b, Mass function not matched for data')
