...
scratch = '/project/projectdirs/m3035/chmodi/cosmo4d/'
train = scratch + 'train/'
data = scratch + 'data/'
...
hdictf = ntools.gridhalos(pm, dpath=scratch +'/data/z%02d/L%04d_N%04d_S%04d_40step/'%(zz*10, bs, sfine*nc, seed), R1=R1, R2=R2, pmesh=False, abund=abund, doexp=doexp, mexp=mexp, cc=cc, stellar=stellar)[1]
...
#
    ...
    fpath = './tmpfigs/' #mtpath + 'hist_M%02d_na.txt'%(10*np.log10(M0))
    ...
    for j, sg in enumerate(sgs):
        ...
        for i, res in enumerate(fits[j]):
            tosave.append([msave[i], msave[i+1], res.x[1], res.x[2]])
        #path = mtpath + 'hist_M%02d_sg%02d_na.txt'%(10*np.log10(M0), sg*100)
        fpath = './tmpfigs/'
        if truemass: fpath = fpath[:-4] + 'trueM.txt'
        if Rsm !=3 : fpath = fpath[:-4] + 'R%d.txt'%Rsm
        np.savetxt(fpath, np.array(tosave), header ='Mmax, Mmin, mean(offset: data-model), b, Mass function not matched for data')
