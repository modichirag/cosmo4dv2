...
for M0 in mms:
    ...
    scatter = dg.gridscatter((datapR[...]+M0)/(datapR[...].mean()+M0), (predictR[...]+M0)/(predictR[...].mean()+M0), mbins, datapR[...])
    tosave = []
    ...
    for i in range(16):
        ...
        xx, yy, res = dg.fitpdf(scatter[i][1] - scatter[i][0], dg.normal, bins=bins, normed=True, x0=[10, 0., 1])
        axis.plot(xx, dg.normal(xx, *res.x), 'k--', label='%0.2f, %0.2f'%(res.x[1], res.x[2]))
        tosave.append([msave[i], msave[i+1], res.x[1], res.x[2]])
        axis.legend()

    ff = mtpath + 'noisehist_ovdM0_M%02d_na.png'%(10*np.log10(M0))
    if Rsm !=3 : ff = ff[:-4] + '_R%d.png'%Rsm
    fig.savefig(ff)

    fpath = mtpath + 'hist_ovdM0_M%02d_na.txt'%(10*np.log10(M0))
