...
def read27(base, ind=None, pm=None):
    '''
    ...
    if ind is not None:
        ...
        #Read over the whole grid if index are not given
        bs, nc = pm.BoxSize[0], pm.Nmesh[0]
        side = bs/nc
        ind = pm.mesh_coordinates(dtype='i4')
        ind = tuple(ind.T)
        #Extend dim of every grid and copy 0-side to N+1 to make things periodic

    else:
        print('Need either an index of positions or a mesh to generate grid')
        return 0
...
def balancepts(pm, base, blim, hlim=0, ratiohigh=1, ratiolow=1, clsgrid=None, halopos=None, resampler='cic', seed=100):
    '''
    Given a base and classgrid, select positions -
    - above hlim on on classgrid, numbered N
    - N*ratiohigh above lim on base but below hlim on classgrid
    - N*ratiolow below lim on base but below hlim  on classgrid
    halopos is only used, to create clsgrid, if clsgrid is None, o/w its unused 
    '''
    ...
    #Assign classes
    if clsgrid is None:
        clsgrid = pm.paint(halopos, resampler = resampler).value
