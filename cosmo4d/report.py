...
from cosmo4d.lab import mapmass as mapp
...
def evaluate(model, data, M0=0, stoch=False, kmin=None, dk=None):
    ...
    xm = FFTPower(first=FieldMesh(model.mapp/model.mapp.cmean()), second=FieldMesh(data.mapp/data.mapp.cmean()), mode='1d', kmin=kmin, dk=dk)
    ...
    pm1 = FFTPower(first=FieldMesh(model.mapp/model.mapp.cmean()), mode='1d', kmin=kmin, dk=dk)
    pd1 = FFTPower(first=FieldMesh(model.d), mode='1d', kmin=kmin, dk=dk)
    ps1 = FFTPower(first=FieldMesh(model.s), mode='1d', kmin=kmin, dk=dk)

    pm2 = FFTPower(first=FieldMesh(data.mapp/data.mapp.cmean()), mode='1d', kmin=kmin, dk=dk)
