...
class MockModel(base.MockModel):
    def __init__(self, dynamic_model, ppath, mpath, pwidth = None):
        ...
        self.pwidth = pwidth
        with open(mpath + '/hodparams.json') as fp: self.hodparams = json.load(fp)
        self._setup_pNN()
        self._setup_mNN()
        self.galmpars = numpy.loadtxt(mpath + '/galmassparams.txt')
        if self.pm.comm.rank == 0: print('Parameters to get galaxy mass are - ', self.galmpars)
    ...
    def get_code(self):
        ...
        #Galaxy
        code.relu(x='massmesh', y='relumass')
        code.add(x1='relumass', x2=Literal(1.), y='1prelumass')
        code.log(x='1prelumass', y='logmass')
        #
        code.multiply(x1='logmass', x2=Literal(1/numpy.log(10)), y='log10mass')
        code.logistic(x='log10mass', y='cenmask', t=self.hodparams['logMmin'], w=1)
        code.multiply(x1='cenmask', x2='posmesh', y='cenmask')
        #
        code.pow(x='logmass', y='logmcube', power=3)
        code.pow(x='logmass', y='logmsq', power=2)
        code.multiply(x1='logmcube', x2=Literal(self.galmpars[0]), y='logmcube')
        code.multiply(x1='logmsq', x2=Literal(self.galmpars[1]), y='logmsq')
        code.multiply(x1='logmass', x2=Literal(self.galmpars[2]), y='loggalmass')
        code.add(x1='loggalmass', x2='logmcube', y='loggalmass')
        code.add(x1='loggalmass', x2='logmsq', y='loggalmass')
        code.add(x1='loggalmass', x2=Literal(self.galmpars[3]), y='loggalmass')
        code.expon(x='loggalmass', y='galmass')
        #
        code.multiply(x1='cenmask', x2='galmass', y='model')
