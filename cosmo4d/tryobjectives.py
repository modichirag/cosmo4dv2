...
class SmoothedObjective(Objective):
    ...
    """
    def __init__(self, likelihood, mock_model, noise_model, data, prior_ps, sml, noised = 2, smooth=None, M0=1e8, L1=False, offset=False, smoothprior=False):
...
#
    ...
    def get_code(self):
        ...
        code.SmoothedOvdLog(residual='residual', modelsm='modelsm', data=data, M0=self.M0)

        #Likelihood
        #print('Calling likelihood')
        #if self.likelihood == 'Logobjective' : LogObjective(data)
        #if self.likelihood == 'Ovdobjective' : OvdObjective(data)
        #if self.likelihood == 'OvdLogobjective' : OvdLogObjective(data)
        #if self.likelihood == 'LogOvdobjective' : LogOvdObjective(data)
