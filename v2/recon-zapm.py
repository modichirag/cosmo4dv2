...
currfolder = optfolder
optfolder = optfolder + '/pm/'
...
ffile = open(optfolder + 'notes', 'a')
ffile.write('Recon the PM sim with ZA forward model to see how well it reconstructs and maybe then we can add it to annealing')
ffile.close()
...
if inpath is not None:
    ...
    #    .c2r()*0.001
    s_init = BigFileMesh(currfolder + '%d-%0.2f/best-fit/'%(nc, 0), 's').paint()
    sms = smoothings[::-1]
    lit = 100
